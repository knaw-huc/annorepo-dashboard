...
import { removeAbsentValues } from "../../model/query/value/QueryValue.ts";
...
export function AnnotationEditor(props: {
  ...
  const filteredConfigFieldSuggestions = configFields
    .map((field) => ({
      path: field.path,
    ...
    .map((field) => ({
      ...
      suggestions: filterSuggestions(
        field.suggestions.data?.map(toOption) ?? [],
        get(form, field.path),
      ),
    }));
  ...
  const typeFieldSuggestions =
    useContainerFieldDistinctValues<string>(containerName, "type").data?.map(
      toOption,
    ) ?? [];
  const filteredTypeSuggestions = filterSuggestions(
  ...
  const handleSubmit = () => {
    ...
    const toSubmit = removeAbsentValues({ ...form, body: form.body });
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <InputWithLabel
                value={
                  isString(form.target)
                    ? form.target
                    : JSON.stringify(form.target, null, 2)
                }
                label="Target"
                onChange={(update) =>
                  setForm((prev) => ({ ...prev, target: update }))
                }
                className="mt-5"
              />
              ...
              {configFields.map((configField) => {
                const suggestions =
                  filteredConfigFieldSuggestions.find(
                    (cfs) => cfs.path === configField.path,
                  )?.suggestions ?? orThrow("No such path");
