...
const openApiEndpoint =
  process.env.OPEN_API_ENDPOINT || "http://localhost:8080/openapi.json";
const schemaFilename = process.env.RESULT_TYPES_PATH || "../../src/openapi.ts";
...
async function main() {
  ...
  if (!openApiFilename) {
    throw new Error(`Could not extract filename from ${openApiEndpoint}`);
  }
